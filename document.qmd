...
Im Mittel beobachten wir einen Wert von `r mean(x)` mit einer Standardabweichung von `r sd(x)`. Der minimale Wert beträgt `r min(x)` und der maximale Wert `r max(x)`.
